...
def get_full_paths(name_of_class: str)->List[str]:
    dataset_dir = 'dataset'
    full_path = os.path.abspath(dataset_dir)
    class_path = os.path.join(full_path, name_of_class)
    image_names = os.listdir(class_path)
    image_full_paths = [os.path.join(class_path, name) for name in image_names]
    return image_full_paths

def get_relative_paths(name_of_class: str)->List[str]:
    dataset_dir = 'dataset'
    rel_path = os.path.relpath(dataset_dir)
    class_path = os.path.join(rel_path, name_of_class)
    image_names = os.listdir(class_path)
